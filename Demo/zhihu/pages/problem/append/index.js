...
Page({
  ...
  setTextAreaHeight(keyboardHeight) {
    ...
    query.select('#image-container1').boundingClientRect();
    query.select('#bottom-container1').boundingClientRect();
    h -= 40;
    query.exec(res => {
    ...
    wx.chooseVideo({
      ...
      success: res => {
        ...
        commonModel.uploadFile(res.tempFilePath)
          .then(res => {
            this.setTextAreaHeight();
            this.setData({ content_video: res, submitEnabled: true, });
            wx.hideLoading();
          })
          .catch(err => {
            this.setTextAreaHeight();
  ...
  async appendSubmit() {
    ...
    await problemModel.questionAddContent(putData)
